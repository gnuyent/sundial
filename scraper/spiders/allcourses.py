...
class AllCoursesSpider(scrapy.Spider):
    ...
    def __init__(self, period='20204', **kwargs):
        ...
        self.start_urls = [f'https://sunspot.sdsu.edu/schedule/search?mode=browse_by_subject&category=browse_by_subject'
                           f'&period={period}']
        self.period = period
    ...
    def parse(self, response):
        ...
        """
        subject_links = LinkExtractor(process_value=self.process_value,
                                      restrict_xpaths='.//div[@id="browseContainer"]/ul').extract_links(response)
    ...
    def scrape_course_links(self, response):
        ...
        """
        course_links = LinkExtractor(restrict_xpaths='.//div[@class="sectionFieldCourse column"]/a').extract_links(
            response)  # Does not need process_value option because it is automatically included
    ...
    def parse_course_information(self, response):
        ...
        course_labels = [index.replace(' ', '_').replace('#', 'num').lower() for index in course_labels]
        # Remove special-case labels
        course_labels = [label for label in course_labels if label not in ('meetings', 'footnotes')]
        course_text = clean(sel.xpath('.//td[@class="sectionDetailContent"]/text()').getall(), True)
        ...
        item['id'] = item['period'] + '-' + item['course'].replace('-', '').replace(' ', '') + '-' + item['section'] \
