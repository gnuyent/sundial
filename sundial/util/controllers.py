...
class ScheduleController:
    ...
    def generate_schedules(self):
        ...
        for course in self.course_list:
            meetings = cursor.execute(f"""SELECT course.id, meeting.days, meeting.hours FROM course, meeting
                        WHERE meeting.course_id = course.id
                        AND course.course LIKE ('{course}')""").fetchall()  # Get every day/time course meets
        ...
        while idx < len(all_classes) - 1:
            ...
            if focus_class == comparison_class and identical_overlap(focus_class, comparison_class):
                all_classes.pop(idx)
            idx += 1

        schedule_tuples = list(itertools.combinations(all_classes, len(self.course_list)))
        schedule_tuples[:] = [unique_schedule for unique_schedule in schedule_tuples if not
                              contains_duplicates(unique_schedule)]  # Remove same course from same schedule
    ...
    def iterate(self):
        [schedule.reset_fitness() for schedule in self.schedules]  # reset fitness levels
