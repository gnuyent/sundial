...
schedule_parameters = {
    ...
    'maximum_time_distance': time(4),  # datetime.time object in hours, minutes of maximum distance (inclusive)
    'bad_day': 'TTH',  # date string in chronological order to dislike
    'earliest_time': time(10),  # datetime.time object in hours, minutes of earliest start time (exclusive)
    'latest_time': time(18),  # datetime.time object in hours, minutes of latest end time (exclusive)
    'waitlist': True  # Allow waitlisted classes
}
controller = ScheduleController(schedule_parameters, 'A S-92A', 'A S-200A', 'CS-310', 'CS-320', 'ENS-331', 'MATH-245',
                                'MATH-254')
